"""
Inference script for Flight Rebooking using HuggingFace-routed models.

Prerequisites:
  1. Create a .env file at repo root (see .env.example)
  2. Start the env server:  uv run server
  3. Run this script:       python inference.py

Environment variables (all optional — defaults target HF router):
  API_BASE_URL  – OpenAI-compatible base URL (default: https://router.huggingface.co/v1)
  MODEL_NAME    – model to use             (default: Qwen/Qwen3-4B-Instruct-2507)
  HF_TOKEN      – HuggingFace API token    (also read from API_KEY)
...
MODEL_NAME = os.getenv("MODEL_NAME") or "Qwen/Qwen3-4B-Instruct-2507"
...
def get_agent_action(
    ...
    """Call the HF-routed model to get the next action.
...
async def main() -> None:
    print("=" * 60, flush=True)
    print("Flight Rebooking — HuggingFace Inference (CoT mode)", flush=True)
    print(f"  API endpoint    : {API_BASE_URL}", flush=True)
    ...
    if not API_KEY:
        print(
            "[ERROR] No API key found. Set HF_TOKEN or API_KEY in your "
