...
# LLM call
# ---------------------------------------------------------------------------
def get_agent_action(model: genai.GenerativeModel, obs, conversation_history: list, step: int) -> Optional[dict]:
    ...
    debug_file_path = debug_dir / f"step_{step}_{timestamp}.txt"
    
    debug_content = []
    
    # Prepend the system prompt for complete context transparency
    debug_content.append(f"[SYSTEM]\n{SYSTEM_PROMPT}\n")
    
    # 1. Format the conversation messages sent to the LLM
    for msg in messages:
        role = msg["role"].upper()
        parts_text = "\n".join(msg["parts"])
        debug_content.append(f"[{role}]\n{parts_text}\n")
    ...
    try:
        ...
        try:
            ...
            # 2a. Add the full raw response
            debug_content.append("========== MODEL RESPONSE (RAW) ==========\n")
            debug_content.append(response_text + "\n")
            
            parsed = parse_llm_response(response_text)
            
            # 2b. Add the parsed and extracted piece that gets passed down
            debug_content.append("\n========== MODEL RESPONSE (PARSED) ==========\n")
            debug_content.append(json.dumps(parsed, indent=2) if parsed else "None")
            debug_content.append("\n")
        except ValueError as val_ext:
            debug_content.append("========== MODEL RESPONSE (COMPLETION STRUCTURE) ==========\n")
            debug_content.append(str(completion) + "\n")
            raise val_ext
...
async def main() -> None:
    ...
    try:
        ...
        for step in range(1, MAX_STEPS + 1):
            ...
            action_dict = get_agent_action(model, obs, conversation_history, step)
